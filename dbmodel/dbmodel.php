...
function readAllNews() 
{
	...
	while ($row = mysqli_fetch_assoc($result)) {
			...
			$date = $row['created'];
			$post = $row['post'];
			$html .= "$post<a href='ADMIN_delete.php?id=$id&table=news' class='btn btn-danger btn-sm'>Delete</a><a href='ADMIN_update.php?id=$id&table=news' class='btn btn-info btn-sm'>Update</a>";
			$html .= "<hr>";	
	}
	closeconnection($connection);
	return $html;
...
function readAllEvents() 
{
	...
	while ($row = mysqli_fetch_assoc($result)) {
			...
			$events .= "$post<a href='ADMIN_delete.php?id=$id&table=events' class='btn btn-danger btn-sm'>Delete</a><a href='ADMIN_update.php?id=$id&table=events' class='btn btn-info btn-sm'>Update</a>";
			$events .= "<hr>";	
	}
	closeconnection($connection);
	return $events;
